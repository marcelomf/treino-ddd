...
export class ProviderRepository {
    ...
    async saveDb(providerDto: ProviderDTO) {
        await this.providerDao.saveDb(providerDto);
    }
    ...
    async findByIdDb(providerId: string): Promise<ProviderDTO> {
        return await this.providerDao.findByIdDb(providerId) as unknown as ProviderDTO;
    }

    async findAllDb(): Promise<ProviderDTO[]> {
        let result = await this.providerDao.findAllDb() as unknown as ProviderDTO[];
        return result;
    }
